...
export function EpicChildTable({ rows, onOpenCard }: { rows: EpicChild[]; onOpenCard: (slug: string) => void }) {
  ...
  return (
    <div className="border border-[color:var(--epic-edge)]">
      ...
      >
        {COLUMNS.map(col => (
          <span key={col} className="text-[9px] font-mono tracking-wider text-muted-foreground/50">
            {col}
          </span>
        ))}
      </div>
      ...
      {rows.length > HEAD && (
        <button
          ...
          className="w-full px-2 py-1 text-left text-[10px] font-mono text-muted-foreground/50 hover:text-foreground border-t border-[color:var(--epic-edge)] transition-colors"
        >
