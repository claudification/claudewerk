...
import { commitRiverRows, type RiverProjectLook, type RiverRow } from '@/lib/wall/commit-river'
...
function useProjectLook(): (uri: string) => RiverProjectLook {
  const projectSettings = useConversationsStore(s => s.projectSettings)
  return useMemo(() => {
    const cache = new Map<string, RiverProjectLook>()
    return (uri: string) => {
      const hit = cache.get(uri)
      if (hit) return hit
      const settings = projectSettings[projectIdentityKey(uri)]
      const look: RiverProjectLook = {
