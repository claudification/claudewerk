...
const LANES: { key: keyof EpicInspectPlan; glyph: string; tone: string; label: string }[] = [
  ...
  { key: 'waitingOnDeps', glyph: '○', tone: 'text-muted-foreground/40', label: 'waiting' },
]
...
function CardLine({ card, glyph, tone, label }: { card: EpicInspectCard; glyph: string; tone: string; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2 py-0.5 text-[11px]">
      ...
      <span className={cn('flex-1 min-w-0 truncate', label === 'waiting' && 'text-muted-foreground/55')}>
        {card.id}
      </span>
      <span className="text-meta text-muted-foreground/45 shrink-0">{waiting}</span>
    </div>
  )
}
...
export function OverseerDag({ plan }: { plan: EpicInspectPlan | null }) {
  if (!plan) {
    return (
      <Block title="DAG">
        <div className="text-[11px] text-muted-foreground/55 italic">
  ...
  return (
    <Block title="DAG" extra={<span>{plan.children} cards</span>}>
      ...
      {lanes.length === 0 && !plan.idleReason && (
        <div className="text-[11px] text-muted-foreground/55 italic">Nothing in any lane.</div>
      )}
      {lanes.map(lane =>
        lane.cards.map(card => (
