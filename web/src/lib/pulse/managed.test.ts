...
describe('managedInfo', () => {
  ...
  it('marks every epic seat, whatever its role', () => {
    for (const role of ['werk-master', 'werk-worker', 'werk-verifier'] as const) {
  ...
  it('groups epic seats by epicId, not by conversation parentage', () => {
    ...
    const a = managedInfo(conv({ epic: { epicId: 'ep_1', role: 'werk-worker', cardId: 'c1', gen: 1 } }))
    const b = managedInfo(conv({ epic: { epicId: 'ep_1', role: 'werk-verifier', cardId: 'c1', gen: 1 } }))
  ...
  it('prefers the epic tag when a conversation somehow carries both', () => {
    const info = managedInfo(
      conv({ epic: { epicId: 'ep_1', role: 'werk-master', gen: 1 }, nightshift: { runId: 'r', taskId: 't' } }),
    )
    expect(info?.kind).toBe('epic')
  })
  ...
  it('does NOT key off anything the conversation could write about itself', () => {
    ...
    // A liveStatus/title/agentName claiming to be a werk-master proves nothing.
    const impostor = conv({
      title: '[epic ep_9] werk-master',
      agentName: 'werk-master',
