...
import { createMemoryScheduledTaskStore } from './scheduled-tasks'
  ...
  return {
    ...
    update(id, patch: ConversationPatch) {
      ...
      for (const [k, v] of Object.entries(patch)) {
        // `undefined` = not patched. `null` = explicit clear -- store it as
        // `undefined` so the record matches what SQLite returns for a NULL
        // column (rowToRecord maps NULL -> undefined).
        if (v === null) delete (s as unknown as Record<string, unknown>)[k]
        else if (v !== undefined) (s as unknown as Record<string, unknown>)[k] = v
      }
    },
    ...
    listRecentByScope(scope, filter) {
      ...
      const withinWindow = results.filter(s => (s.lastActivity ?? 0) >= cutoff).length
      return results.slice(0, recentLimit(window, withinWindow)).map(toSummary)
    },
    ...
    countByScopeAndStatus(status) {
      ...
      for (const s of conversations.values()) {
        if (!status.includes(s.status)) continue
    ...
    liveStatusByScope(scope) {
      const out: ConversationLiveStatusRecord[] = []
      for (const s of conversations.values()) {
        if (s.scope !== scope) continue
        const meta = s.meta as { liveStatus?: LiveStatus; lastInputAt?: number } | undefined
        if (!meta?.liveStatus) continue
        out.push({ id: s.id, liveStatus: meta.liveStatus, lastInputAt: meta.lastInputAt })
      }
      return out
    },
    ...
    listScopes() {
      ...
      for (const s of conversations.values()) scopes.add(s.scope)
      return [...scopes].sort()
    },
...
/** Order search hits in place: bm25-surrogate rank by default, newest-first on
 *  recency -- mirrors the sqlite driver's `ORDER BY rank` / `timestamp DESC, id DESC`. */
function sortSearchHits(hits: SearchHit[], sort?: SearchSort): void {
  if (sort === 'recency') hits.sort((a, b) => b.timestamp - a.timestamp || b.id - a.id)
  else hits.sort((a, b) => a.rank - b.rank)
}
...
/** The conversation buckets a scan is allowed to read: one, a named set, or all. */
function selectSources(
  entries: Map<string, TranscriptEntryRecord[]>,
  opts: ScanFilters | undefined,
): Array<[string, TranscriptEntryRecord[]]> {
  if (opts?.conversationId) return [[opts.conversationId, entries.get(opts.conversationId) ?? []]]
  if (!opts?.conversationIds?.length) return [...entries.entries()]
  const idSet = new Set(opts.conversationIds)
  return [...entries.entries()].filter(([cid]) => idSet.has(cid))
}

/** Walk every entry a search/browse call is allowed to consider, yielding
 *  `[conversationId, entry]`. Applies the conversation and type filters so both
 *  callers agree on scope; only the per-entry verdict differs. */
function* scanEntries(
  entries: Map<string, TranscriptEntryRecord[]>,
  opts: ScanFilters | undefined,
): Generator<[string, TranscriptEntryRecord]> {
  const typeSet = opts?.types?.length ? new Set(opts.types) : null
  for (const [conversationId, arr] of selectSources(entries, opts)) {
    for (const e of arr) {
      if (typeSet && !typeSet.has(e.type)) continue
      yield [conversationId, e]
    }
  }
}

/** The identity half of a SearchHit -- everything except rank and snippet, which
 *  are the only fields search and browse disagree on. */
function hitBase(conversationId: string, e: TranscriptEntryRecord): Omit<SearchHit, 'rank' | 'snippet'> {
  return {
    id: e.id,
    conversationId,
    seq: e.seq,
...
/** A `...match...` snippet window (±32 chars) around the match index, with ellipses
 *  only where the text is actually clipped -- the substring analogue of FTS5 snippet(). */
function buildMatchSnippet(text: string, idx: number, matchLen: number): string {
  const start = Math.max(0, idx - 32)
  const end = Math.min(text.length, idx + matchLen + 32)
...
function createTranscriptStore(): TranscriptStore {
  ...
  // Seq counters keyed PER SCOPE (`conversationId\0agentId`) so parent and each
  // agent sub-stream advance independently -- mirrors the sqlite driver.
  ...
  function nextSeq(conversationId: string, agentId: string | null | undefined): number {
  ...
  function scopeMatches(entry: TranscriptEntryRecord, agentId: string | null | undefined): boolean {
    if (agentId === undefined) return true
    return agentId === null ? !entry.agentId : entry.agentId === agentId
  }
  ...
  return {
    append(conversationId, syncEpoch, inputEntries) {
      ...
      for (const e of inputEntries) {
        // Already stored (replay / full-file re-read): report the EXISTING seq
        // and do not consume a new one. Mirrors the sqlite driver exactly --
        // both must agree, they are the same contract to the ingest path.
        const existing = arr.find(x => x.uuid === e.uuid)
        if (existing) {
          results.push({ uuid: e.uuid, seq: existing.seq, inserted: false })
          continue
        }
        const seq = nextSeq(conversationId, e.agentId)
        arr.push({
          id: nextId(),
          conversationId,
          seq,
        ...
        results.push({ uuid: e.uuid, seq, inserted: true })
      }
      return results
    },
    ...
    getPage(conversationId, opts) {
      ...
      if (opts.agentId !== undefined) {
        arr = arr.filter(e => scopeMatches(e, opts.agentId))
      }
    ...
    getLatest(conversationId, limit, agentId) {
      ...
      if (agentId !== undefined) {
        arr = arr.filter(e => scopeMatches(e, agentId))
      }
      return arr.slice(-limit)
    },

    getSinceSeq(conversationId, sinceSeq, limit, agentId) {
      const arr = getEntries(conversationId).filter(e => scopeMatches(e, agentId))
      const maxSeq =
        agentId === undefined
          ? maxSeqForConversation(conversationId)
          : (seqCounters.get(scopeKey(conversationId, agentId)) ?? 0)
    ...
    getLastSeq(conversationId) {
      return maxSeqForConversation(conversationId)
    },

    hasUuid(conversationId, uuid) {
      return getEntries(conversationId).some(e => e.uuid === uuid)
    },
    ...
    search(query, opts) {
      ...
      for (const [conversationId, e] of scanEntries(entries, opts)) {
        const text = JSON.stringify(e.content)
        const idx = text.toLowerCase().indexOf(q)
        if (idx === -1) continue
        hits.push({
          ...hitBase(conversationId, e),
          rank: -1 / (text.length + 1), // shorter matches rank higher (less-negative bm25 surrogate)
          snippet: buildMatchSnippet(text, idx, q.length),
        })
      }
      sortSearchHits(hits, opts?.sort)
    ...
    browse(opts) {
      ...
      const hits: SearchHit[] = []
      for (const [conversationId, e] of scanEntries(entries, opts)) {
        hits.push({ ...hitBase(conversationId, e), rank: 0, snippet: buildBrowsePreview(e.content) })
      }
      // Always newest-first -- a listing with no query has no relevance order.
      sortSearchHits(hits, 'recency')
      return hits.slice(offset, offset + limit)
    },
    ...
    getWindow(conversationId, opts) {
      ...
      if (centerSeq == null) {
        ...
        return tail == null ? [] : arr.slice(-tail)
      }
...
function createMessageStore(): MessageStore {
  ...
  return {
    ...
    dropOldest(scope) {
      const now = Date.now()
      const idx = queue.findIndex(m => m.toScope === scope && m.expiresAt > now)
...
function sampleInWindow(s: MemTokenSample, f: TokenBucketFilter): boolean {
  ...
  if (f.conversationId && s.conversationId !== f.conversationId) return false
  return true
}
  ...
  return {
    ...
    queryBuckets(filter) {
      ...
      for (const s of samples) {
        ...
        b.cacheWrite5mTokens += s.cacheWrite5mTokens
        b.cacheWrite1hTokens += s.cacheWrite1hTokens
        b.samples++
        buckets.set(key, b)
      }
...
export function createMemoryDriver(): StoreDriver {
  return {
    ...
    scheduledTasks: createMemoryScheduledTaskStore(),
