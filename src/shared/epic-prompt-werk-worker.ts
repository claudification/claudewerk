...
export function buildWerkWorkerPrompt(ctx: WerkWorkerPromptCtx): string {
  ...
  return [
    ...
    '   reason is ground truth -- fix the cause, never route around it. It also DERIVES suites from what your',
    '   diff touched and runs them on top of `test_cmd` (`web/` or `src/shared/` obliges the web suite), and it',
    '   refuses a branch that has not merged `main` -- so `git merge main` and re-run before you move the card.',
    '   The gate can also be OFF, in which',
