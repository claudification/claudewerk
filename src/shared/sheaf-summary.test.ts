...
import { summarizeSheaf } from './sheaf-summary'
import type { SheafNode, SheafProject, SheafResponse } from './sheaf-types'
...
describe('summarizeSheaf', () => {
  ...
  test('carries the project URI, not just the colliding label', () => {
    const s = summarizeSheaf(sheaf([project('rclaude', 1, [node()])]))
    expect(s.projects[0]?.projectUri).toBe('claude:///rclaude')
  })
