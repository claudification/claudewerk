...
function unlandedBlock(ctx: WerkMasterPromptCtx): string {
  ...
  return [
    ...
    '',
    `  The run's target is \`${ctx.run.target}\`. The engine DERIVES this from the commit ledger every beat --`,
...
function theJob(ctx: WerkMasterPromptCtx): string {
  ...
  return [
    ...
    '',
    '   THIS IS NOT ADVICE. The engine reads the commit ledger every beat and knows exactly which card',
    '   branches are not on main. It is the ONLY job in this list nothing else in the run performs -- a',
