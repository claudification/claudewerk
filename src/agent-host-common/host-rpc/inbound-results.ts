...
export function dispatchHostRpcResult(msg: Msg, pending: PendingCallbacks, diag: DiagSink): boolean {
  ...
  // SOTU MCP read/write replies (Phase 5 + the Phase-7 configure/eval RPCs). The
  // sotu_contribution / sotu_updated broadcasts carry no requestId, so only the
  // matched RPC replies forward.
  if (
