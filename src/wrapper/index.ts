...
async function main() {
  ...
  writeFileSync(
    ...
    [
      ...
      '# Task Notes (rclaude)',
      '',
      'This project has a structured task system at `.claude/.rclaude/tasks/`.',
      'Tasks are markdown files with YAML frontmatter. Status is determined by folder.',
      '',
      '```',
      '.claude/.rclaude/tasks/',
      '  open/           # New tasks, not started yet',
      ...
      'refs: [src/foo.ts, docs/x.md]  # optional, related files',
      'created: 2026-04-07T10:00:00Z  # auto-set',
      '---',
      'Body text. Full markdown.',
      '```',
      '',
      '**Commands:**',
      '- "check task X" / "look at task X" -> find in open/, then in-progress/, then done/. Read and present it.',
      '- "start task X" -> mv open/X.md in-progress/X.md',
      '- "done with task X" -> mv in-progress/X.md done/X.md',
      '- "archive task X" -> mv done/X.md archived/X.md',
      '- "create task: ..." -> write new .md file to open/ with frontmatter',
      '- When working on a task, update the file: add findings, change priority, add refs, refine the body.',
      '- A file watcher pushes changes to the dashboard kanban board automatically.',
      '',
      ...(channelEnabled
        ? [
            '',
